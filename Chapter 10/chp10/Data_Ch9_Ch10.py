...
"""
## Just Remove the """ """ to run the code
Note
"""
...
plt.plot(highes_cpi_cl.columns['CPI 2013 Score'], highes_cpi_cl.columns['Total (%)'])
plt.xlabel('CPI Score - 2013')
plt.ylabel('Child Labor Percentage')
plt.title ('CPI & Child Labor Correlation')
plt.show()
...
##Activity 4## pwedi nga indi ni ya pag e erase
with open ('iso-2-cleaned.json', 'rb') as f:
...
ranked = ranked.compute([('country_code',agate.Formula(text_type, get_country_code)),])
